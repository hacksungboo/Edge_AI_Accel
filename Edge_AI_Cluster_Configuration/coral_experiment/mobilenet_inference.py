# code for inferencing with mobilenet using coral edge tpu
...
MODEL_PATH = "./model/mobilenet_v2_1.0_224_quant_edgetpu.tflite"

def load_imagenet_labels():
    labels_path = "./imagenet_labels.txt"
    if not os.path.exists(labels_path):
        raise FileNotFoundError("No imagenet_labels.txt")
...
for i, img in enumerate(random_images):
    ...
    # result
    #label_name = imagenet_labels[pred_label] if pred_label < len(imagenet_labels) else f"Unknown({pred_label})"
    #print(f"[{i+1}/100] Predicted: {label_name}, Confidence: {confidence:.4f}, Time: {inference_time:.4f}s")
