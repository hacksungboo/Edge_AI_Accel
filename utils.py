...
def mobility_update_loop(mobility_manager):
    ...
    while True:
        try:
            mobility_manager.update_mobility(dt=1.0)
            update_count += 1
...
def save_experiment_results(experiment_id, scheduling_decisions, mobility_manager, 
    ...
    # 1. 스케줄링 결정 저장
    with open(f"./results/scheduling_decisions_{experiment_id}.csv", 'w', newline='') as f:
    ...
    # 2. 디바이스 정보 저장
    with open(f"./results/devices_{experiment_id}.csv", 'w', newline='') as f:
        writer = csv.writer(f)
    ...
    with open(f"./results/experiment_summary_{experiment_id}.json", 'w') as f:
